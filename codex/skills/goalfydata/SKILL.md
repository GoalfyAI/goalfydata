...
**本文档的更新机制**: 本文档随插件/技能包分发，且仅在会话开始时加载——更新插件后必须重开会话才生效。当本文档的参数、流程与工具实际行为不符时，优先怀疑本文档已过旧，按常见问题表的对应条目引导用户更新。
...
每建一张表，必须紧接着调 `uds_table_manage(action="create", task_id=<task_id>)` 注册元数据，否则 GoalfyData 控制台和其他 Agent 看不到该表。
...
```
1. uds-cli --task-id <task_id> upload orders.csv --dataset dataset_id → 拿到 workspace_path
...
```
1. 本地写好脚本 → uds-cli --task-id <task_id> upload fetch_orders.py --dataset dataset_id → 拿到 workspace_path
...
2. 上传脚本：uds-cli --task-id <task_id> upload fetch_script.py --dataset dataset_id → workspace_path
...
| 工具或 uds-cli 返回 401/未认证（此前正常） | API Key 已被删除或轮换。最简单方式：引导用户到官网集成页重新复制接入文本发给你（ https://goalfydata.ai/integrations ），按其中安装流程重走一遍。手动方式：引导用户到控制台 https://goalfydata.ai/settings 创建新 Key → `uds-cli login` 重新登录 → 若 MCP 配置的环境变量里存着旧 Key 则一并更新 → 让用户完全重启会话（环境变量优先级高于登录配置，不更新会继续用旧 Key） |
| SKILL 指引与工具实际行为不符（参数报错、流程对不上） | 插件里的本文档可能是旧版。按下方「5.1 SKILL 版本过旧的更新方法」处理 |

### 5.1 SKILL 版本过旧的更新方法

判断依据（出现任一即怀疑本文档过旧）：工具返回的参数校验错误与本文档描述不符、执行流程与工具实际行为对不上、服务端返回版本过旧类提示。
...
3. 验证：重开会话后 `/mcp` 显示 `goalfydata-mcp` connected + 20 tools，且新文档内容已生效
...
3. 验证：重开会话后 `goalfydata-mcp` 已连接、工具列表 20 个
...
**通用兜底**：任何平台都可以引导用户到官网集成页重新复制接入文本发给你，一步完成更新与重新接入： https://goalfydata.ai/integrations
