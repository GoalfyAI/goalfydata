...
用户提及看板、dashboard、数据应用、数据网站或可视化展示，且数据来源为 GoalfyData 数据集时，应识别为 GoalfyData 应用部署需求，而非常规前端开发任务，须按 4.5 执行，不得自行创建工程。
...
| 说"分享给某人" | 进入分享流程 |
| 用户要求将数据制作为看板/网站/应用 | 数据已存于或即将沉淀至 GoalfyData 数据集时，须走应用部署流程（4.5，见约束 8）；不得本地自建工程或部署至第三方平台 |
...
说明：4.5 流程中的下载模板、本地编码、本地打包等本地操作属于既定环节（MCP 不读写本地文件），不在禁止之列；禁止的是绕开官方模板与部署链路自行搭建。
