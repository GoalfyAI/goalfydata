...
**未持有 API Key，或工具返回未认证时**，按下面模板向用户输出（语言与用户对话一致，非中文用户翻译后输出；作为正文直接输出，一级标题和加粗必须保留），不要自行编造或使用占位 API Key：
...
表中凡涉及需要用户亲自操作的步骤（到官网操作、更新插件、重启应用或会话），转述给用户时必须用一级标题加粗的「需要您操作」格式输出（样式参照前置条件中的 API Key 模板，语言与用户对话一致），不得写成普通句子。
...
先确认用户所在平台，再按对应方式更新。涉及用户亲自操作的步骤，用「需要您操作」一级标题加粗格式输出，语言与用户对话一致。
