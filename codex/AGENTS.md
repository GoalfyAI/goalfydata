...
1. Only run the commands listed here; only write to the installer-owned `~/.goalfy/` directory, the matching shell rc PATH line described in Step 2, `~/.codex/.env` (line-level update), and the Codex plugin directory
...
3. Never print or read the plaintext Key into the conversation; fixed local scripts may move it from `~/.goalfy/config.json` into Codex's environment without echoing it
...
This conversation is running inside Codex's sandbox and approval system, not inside the user's later macOS or Linux terminal. The current session has a built-in scoped escalation path for commands that need network access or writes outside the workspace.

1. Run every check and command in this guide yourself.
2. Treat `Could not resolve host: cdn.goalfydata.ai`, other blocked network access, or a write denial under `~/.goalfy`, the selected shell rc file, `~/.codex/.env`, or the plugin directory as a sandbox restriction first. It is not evidence that the website is down, the API Key is invalid, or the user's normal network is broken.
...
python3 -c 'import os,sys; p=os.path.expanduser("~/.codex/.env"); lines=open(p).read().splitlines() if os.path.exists(p) else []; sys.exit(0 if any(x.startswith("GOALFY_UDS_API_KEY=gfk_") for x in lines) else 1)' # MCP credential configured?
...
Check whether `~/.codex/.env` already matches `~/.goalfy/config.json` without printing either value:

```bash
python3 - <<'PY'
import json, os, sys
home = os.path.expanduser('~')
key = json.load(open(os.path.join(home, '.goalfy', 'config.json'))).get('api_key')
path = os.path.join(home, '.codex', '.env')
lines = open(path).read().splitlines() if os.path.exists(path) else []
stored = next((line.split('=', 1)[1] for line in lines if line.startswith('GOALFY_UDS_API_KEY=')), None)
sys.exit(0 if isinstance(key, str) and key.startswith('gfk_') and stored == key else 1)
PY
```

If the check fails, update the line from the local CLI config without displaying the credential:
...
lines = open(path).read().splitlines() if os.path.exists(path) else []
env_name = 'GOALFY_UDS_API_KEY'
env_prefix = env_name + '='
lines = [line for line in lines if not line.startswith(env_prefix)]
tmp = path + '.tmp'
with open(tmp, 'w') as handle:
    handle.write('\n'.join(lines + [env_prefix + key]) + '\n')
os.chmod(tmp, 0o600)
os.replace(tmp, path)
PY
```

Never print, grep, compare prefixes, or interpolate the credential into a shell command. Preserve every unrelated line and re-run the non-printing equality check before continuing.
...
- Local credential synced to ~/.codex/.env without displaying it
...
| `Could not resolve host: cdn.goalfydata.ai` or installer writes are denied | Treat it as a Codex sandbox restriction. Request scoped network/filesystem escalation and retry the command yourself; do not ask the user to run it in Terminal |
