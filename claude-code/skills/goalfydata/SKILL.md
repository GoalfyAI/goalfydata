...
**获取 API Key**: 到 GoalfyData 控制台「设置 → API Key」创建：https://goalfydata.ai/settings 。明文仅在创建时返回一次，请妥善保存。
...
- 向用户提及获取、核实、管理 API Key 或账号时，附上控制台链接 https://goalfydata.ai/settings
...
每建一张表，必须紧接着调 `uds_table_manage(action="create", task_id=<task_id>)` 注册元数据，否则 GoalfyData 控制台和其他 Agent 无法看到该表。
...
| 10. 生成模板文件 | upload 源表必做：生成 xlsx 模板（第 1 行 snake_case 表头，与 target_columns 一致；表头后 2-3 行真实业务示例值；无聚合行），`uds-cli upload 模板.xlsx --dataset ... --type sample` → 获取 workspace_path | 模板是 Console"下载模板"给用户的参考，只需让用户看清有哪些列、数据大致长什么样——格式宽容由 transform 脚本负责，不对用户做格式要求 |
...
- 表尚未配置脚本（Console 上"替换数据"置灰、或触发报 `SCRIPT_NOT_CONFIGURED`）→ 按 4.1 Step 2.1 的步骤 9-11 补齐脚本、模板与登记后重试
...
| **error_code=SCRIPT_NOT_CONFIGURED** | 表未登记更新脚本（注册未完成）。按 4.1 Step 2.1 步骤 9-11 补齐：写 transform 脚本 + 生成模板 + `uds_table_manage(update, script_file=..., sample_file=..., sources=[{type: upload, entry: transform}])`，完成后 Console"替换数据"自动恢复 |
...
- 报错文案面向最终用户：使用与用户一致的语言（标准模板示例为中文，英文用户的数据集用英文），带行号列名与修正建议（Console 会原样展示 error），禁止只透传底层技术异常原文

#### 模板文件规范（upload 源必备）

`sample_file` 是 Console"下载模板"按钮给用户的格式参考，与 transform 脚本成对维护：
...
表中凡涉及需要用户亲自操作的步骤（到官网/控制台操作、更新插件、重启应用或会话），转述给用户时必须用一级标题加粗的「需要你操作」格式输出（样式参照前置条件中的 API Key 模板），不得写成普通句子。
...
| 工具或 uds-cli 返回 401/未认证（此前正常） | API Key 已被删除或轮换。最简单方式：引导用户到官网集成页重新复制接入文本发送给你（ https://goalfydata.ai/integrations ），按其中安装流程重新执行一遍。手动方式：引导用户到控制台 https://goalfydata.ai/settings 创建新 Key → `uds-cli login` 重新登录 → 若 MCP 配置的环境变量中保存的仍是旧 Key 则一并更新 → 让用户完全重启会话（环境变量优先级高于登录配置，不更新会继续用旧 Key） |
